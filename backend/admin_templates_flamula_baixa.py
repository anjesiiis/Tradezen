...
router = APIRouter(
    prefix="/admin/templates-flamula-baixa",
    tags=["admin-templates-flamula-baixa"],
    dependencies=[Depends(require_admin)],
)


# Os 6 pontos e as regras de validação vivem em bandeira_pontos.py
# (mesmas regras da bandeira — muda só a tabela e a direção).
...
def listar_templates(request: Request):
    resp = supabase.table("templates_flamula_baixa").select(_COLUNAS_LISTA).order("criado_em", desc=True).execute()
...
def obter_template(request: Request, template_id: int):
    resp = supabase.table("templates_flamula_baixa").select("*").eq("id", template_id).limit(1).execute()
    if not resp.data:
...
def criar_template(request: Request, payload: TemplateCreate):
    ...
    resp = supabase.table("templates_flamula_baixa").insert(body).execute()
    return {"status": "ok", "template": resp.data[0]}
...
def atualizar_template(request: Request, template_id: int, payload: TemplateUpdate):
    ...
    resp = supabase.table("templates_flamula_baixa").update(body).eq("id", template_id).execute()
    if not resp.data:
        raise HTTPException(status_code=404, detail="Template não encontrado.")
    return {"status": "ok", "template": resp.data[0]}
...
def remover_template(request: Request, template_id: int):
    resp = supabase.table("templates_flamula_baixa").delete().eq("id", template_id).execute()
    if not resp.data:
        raise HTTPException(status_code=404, detail="Template não encontrado.")
    return {"status": "ok"}
