...
# ──────────────────────────────────────────────────────────────
def _calcular_atr(candles: List[Dict], periodo: int = 14) -> List[float]:
    n = len(candles)
    atrs = [0.0] * n
    if n < 2:
        return atrs
...
# ──────────────────────────────────────────────────────────────
def _encontrar_pivos(candles: List[Dict], janela: int = 5):
    atrs = _calcular_atr(candles)
    topos, fundos = [], []
    n = len(candles)
...
def detectar_padroes_classicos(candles: List[Dict], janela: int = 5) -> List[Dict]:
    if not candles or len(candles) < (janela * 2 + 3):
        return []
    topos, fundos = _encontrar_pivos(candles, janela=janela)
